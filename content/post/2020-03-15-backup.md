...
Windows下的备份跟Linux很类似，这里使用DOS下的`robocopy`命令进行同步。这里有一个[backup.txt](https://core-man.github.io/blog/scripts/backup.txt)文件可以参考，将后缀txt给成bat，就变成了bat脚本，双击bat脚本就直接运行了。这里需要认真阅读txt格式的原文件，并将里面的参数改成自己需要的。
...
更多Mac文件系统参考[磁盘工具使用手册](https://support.apple.com/zh-cn/guide/disk-utility/dsku19ed921c/mac)、[谈谈Mac OS的文件系统](https://zhuanlan.zhihu.com/p/33656976)以及[How-To Geek](https://www.howtogeek.com/331042/whats-the-difference-between-apfs-macos-extended-hfs-and-exfat)
